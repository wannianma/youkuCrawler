...
def _insert_video_data(cursor,kw):
    try:
        ...
        cursor.execute("INSERT INTO " +config.DB_TABLE_VIDEO+" (`catid`, `typeid`, `title`, `style`, `thumb`, `keywords`, `description`, `posids`, `url`, `listorder`, `status`, `sysadd`, `islink`, `username`, `inputtime`, `updatetime`,`vision`,`video_category`,`anchor`) VALUES (%s, '0', %s, '', %s, %s, %s, '0', '', '0', '99', '1', '0', 'spider', %s, %s,'1','1',%s)",(kw['catid'], kw['title'], kw['thumb'], kw['keywords'], kw['keywords'], kw['publishtime'], kw['publishtime'],kw['anchor']))
