...
class BiliExtractor(BaseExtractor):
    def __init__(self, zhubo):
        super(BiliExtractor, self).__init__(zhubo)
    ...
    def get_single_list(self, channel_url, is_all=False):
        ...
        # 下一页地址
        next_page_url = None
        html = self.get_html_by_phantomjs(channel_url)
        if html.strip() != '':
            soup = BeautifulSoup(html, 'html.parser', from_encoding='utf-8')
            # 获取下一页地址
            next_page = soup.find(class_='page-link next')
	    print next_page;
...
if __name__ == '__main__':
    zhubo = [{'url':'http://space.bilibili.com/17819768/#!/video'}]
    bili = BiliExtractor(zhubo)
    for zb in zhubo:
        arr = bili.get_single_list(zb['url'])
